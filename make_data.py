...
class Dataset():
    ...
    def create_tfrecords_file(self, data, output_filename, example_fn):
        ...
        with tf.python_io.TFRecordWriter(output_filename) as writer:
            ...
            list(map(lambda item: writer.write(item),
                     examples))
...
def create_embeddings(vocab_array, word2vec, embedding_dim):
    ...
    np.save(tf.flags.FLAGS.initial_embeddings_path, initial_embeddings)
...
if __name__ == "__main__":
    ...
    dataset.write_vocabulary(vocab, os.path.join(
        tf.flags.FLAGS.output_dir, 'vocabulary.txt'))

    # Save vocab processor
    vocab.save(os.path.join(
        tf.flags.FLAGS.output_dir, 'vocab_processor.bin'))
    ...
    dataset.create_tfrecords_file(input, os.path.join(
        tf.flags.FLAGS.output_dir, "validation.tfrecords"),
    ...
    dataset.create_tfrecords_file(input, os.path.join(
        tf.flags.FLAGS.output_dir, "test.tfrecords"),
    ...
    dataset.create_tfrecords_file(input, os.path.join(
        tf.flags.FLAGS.output_dir, "train.tfrecords"),
