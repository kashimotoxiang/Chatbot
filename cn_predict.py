...
def get_features(context, utterances):
    ...
    context_matrix = np.array(
        [context_matrix_for_one for i in range(len(utterances))])
    # print("context_matrix_shape{}".format(context_matrix.shape))
    utterance_matrix = np.array(
        [next(vp.transform([i])).tolist() for i in utterances])
    # print("utterance_matrix_shape{}".format(utterance_matrix.shape))
    context_len = np.array([len(next(vp._tokenizer([context])))
                            for i in range(len(utterances))])
    utterance_len = np.array(
        [len(next(vp._tokenizer([utterances[i]]))) for i in range(len(utterances))])
    features = {
...
if __name__ == "__main__":
    ...
    probs = list(estimator.predict(
        input_fn=lambda: get_features(INPUT_CONTEXT, POTENTIAL_RESPONSES)))
    result = {}
    ...
    print("We have found the answer for you:\n{}".format(
        dataset.raw_data[result[0][0]]["answer"]))
