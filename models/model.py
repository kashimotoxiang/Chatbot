...
def dual_encoder_model(
    ...
    with tf.variable_scope("prediction"):
        ...
        logits = tf.squeeze(logits, [2], name="logits")  # 删除第三个维度?
        # print('squeeze logits shape {}'.format(logits.get_shape()))
        # Apply sigmoid to convert logits to probabilities
        probs = tf.sigmoid(logits, name="probs")  # 没有问题，是sigmoid不是softmax
        ...
        losses = tf.nn.sigmoid_cross_entropy_with_logits(
            logits=logits, labels=tf.to_float(targets), name="losses")
