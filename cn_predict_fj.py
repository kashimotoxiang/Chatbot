...
cn_hparams.model_dir_generator(
    use_word2vec, model.name, RNNInit.__name__, is_bidirection)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if input_question.isdigit():
            ...
            right_answer_rank = np.where(
                sort_probs_idx == input_question)[0][0]
            print('question: {}'.format(raw_data[input_question]["question"]))
            print('right answer: {}'.format(
                raw_data[input_question]["answer"]))
            print('The most likely answer: {}'.format(
                raw_data[max_index]["answer"]))
